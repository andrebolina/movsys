...
class MoviesList extends Component {
  ...
  handleSort() {
    ...
    this.props.history.push(
      `/?title=${this.inputTitle.value}&year=${
        this.inputYear.value
      }&order=${params.get('order')}&sort=${newSort}`
    );
  }
  ...
  handlerOrder(value) {
    ...
    this.props.history.push(
      `/?title=${this.inputTitle.value}&year=${
        this.inputYear.value
      }&order=${value}&sort=${params.get('sort')}`
    );
  }
  ...
  search(e) {
    ...
    if (searchTitle.length !== 0) {
      ...
      history.push(
        `/?title=${searchTitle}&year=${seartchYear}&order=title&sort=asc`
      );
    } else {
      history.push(`/?error`);
    }
  ...
  render() {
    ...
    if (movies && movies.length > 0) {
      movieList = movies.sort(this.sort_by(order, reverse)).map((item, i) => {
        return <MovieCard key={i} infos={item} />;
      });
    } else {
      movieList = (
    ...
    return (
      <>
        ...
        >
          ...
          />
          <Fab color="primary" onClick={this.search.bind(this)}>
            <SearchIcon />
          </Fab>
        ...
        {params.get('title') || formError ? (
          ...
          <div className={classes.container}>
            ...
            <div className={classes.instructionsMessage}>
              Use the fields above to search for a movie.
            </div>
          </div>
        )}
      </>
...
const MoviesListComponent = connect(
  mapStateToProps,
  mapDispatchToProps
)(MoviesList);
